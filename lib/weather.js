...
export async function fetchWeather({ lat, lon } = {}) {
  const latVal = lat ?? process.env.DEFAULT_LAT;
  const lonVal = lon ?? process.env.DEFAULT_LON;
  if (!latVal || !lonVal) {
    throw new Error("No coordinates provided and no DEFAULT_LAT/DEFAULT_LON set");
  }
