...
export default function FundsPage() {
  ...
  const downloadPdf = async (id, schemeName) => {
    setDownloadingId(id);
    try {
      const res = await fetch(`/api/funds/document?id=${id}`);
      ...
      link.download = data.documentName || `${schemeName || "fund"}.pdf`;
  ...
  return (
    <div className="pt-36 max-w-7xl mx-auto px-4 pb-12">
      ...
      {loading && (
        <p className="text-gray-500 text-center py-8">Loading fund data...</p>
      )}
              ...
              {funds.map((item) => (
                ...
                >
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">
                    {formatFundAmount(item.totalAllocation)}
                  </td>
                  <td className="px-3 py-3 whitespace-nowrap">
                    {formatFundAmount(item.amountReceived)}
                  </td>
                  <td className="px-3 py-3 whitespace-nowrap">
                    {formatFundAmount(item.balance)}
                  </td>
                  <td className="px-3 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getFundStatusColor(item.status)}`}
                    >
                      {item.status}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">
                    {formatFundDate(item.startDate)}
                  </td>
                  <td className="px-3 py-3 whitespace-nowrap">
                    {formatFundDate(item.endDate)}
                  </td>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-xs text-gray-500">
                    {formatFundLastUpdated(item.updatedAt)}
                  </td>
                </tr>
              ))}
            </tbody>
